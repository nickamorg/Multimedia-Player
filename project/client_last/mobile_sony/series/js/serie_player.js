...
function display_serie_details(serie_id) {
    ...
    $('#serie_info').html(`
    ${series[serie_id].release.split(" ")[0]} • ${serie.duration} • 
    <span><img width="30" height="15" src="../ressrc/series_images/imdb.png"></span> ${serie.rate} ${serie.genre}`);
    ...
    for(let i = 0; i < series.crowd; i++) {
        if (series["id" + i].release.split(" ")[0] === (new Date()).getFullYear().toString()) {
            $("#series_news_content").append(`
                <div class="col-xs-6" onclick="display_serie_details('id' + ${i})">
                    <img class="img-serie_gen" src="../ressrc/series_images/${series["id" + i].img}"/>
                    <h3>${series["id" + i].title}</h3>
                    <h5>${series["id" + i].release}</h5>
                </div>
            `)
        }
    }
        ...
        for(let j = 0; j < series.crowd; j++) {
            if(current_series.indexOf("id" + j) == -1 && series["id" + j].genre.indexOf(genres[i]) > -1) {
                $("#series_genres_content").append(`
                    <div class="col-xs-6" onclick="set_series_by_genre('${genres[i]}')">
                        <img class="img-serie_gen" src="../ressrc/series_images/${series["id" + j].img}"/>
                        <h3>${genres[i]}</h3>
                    </div>
                `)
