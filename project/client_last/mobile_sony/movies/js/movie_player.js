...
function display_movie_details(movie_id) {
    ...
    $('#movie_info').html(`
    ${movies[movie_id].release.split(" ")[2]} • ${movie.duration} • 
    <span><img width="30" height="15" src="../ressrc/movies_images/imdb.png"></span> ${movie.rate} ${movie.genre}`);
    ...
    for(let i = 0; i < movies.crowd; i++) {
        if (movies["id" + i].release.split(" ")[2] === (new Date()).getFullYear().toString()) {
            $("#movies_news_content").append(`
                <div class="col-xs-6" onclick="display_movie_details('id' + ${i})">
                    <img class="img-movie_gen" src="../ressrc/movies_images/${movies["id" + i].img}"/>
        ...
        for(let j = 0; j < movies.crowd; j++) {
            if(current_movies.indexOf("id" + j) == -1 && movies["id" + j].genre.indexOf(genres[i]) > -1) {
                $("#movies_genres_content").append(`
                    <div class="col-xs-6" onclick="set_movies_by_genre('${genres[i]}')">
                        <img class="img-movie_gen" src="../ressrc/movies_images/${movies["id" + j].img}"/>
                        <h3>${genres[i]}</h3>
                    </div>
                `)
